...
export const TableHeader = React.forwardRef<HTMLTableSectionElement, React.HTMLAttributes<HTMLTableSectionElement>>(
  ({ className, ...props }, ref) => (
    <thead ref={ref} className={cn("bg-muted/40 border-b border-border/60 [&_tr]:border-b-0", className)} {...props} />
  )
);
...
export const TableHead = React.forwardRef<HTMLTableCellElement, React.ThHTMLAttributes<HTMLTableCellElement>>(
  ({ className, ...props }, ref) => (
    <th
      ref={ref}
      className={cn(
        "h-11 px-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground align-middle [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
